...
You do not need to shell out to the CLI to stop an instance, and you do not
...
the same ``port``, ``pid``, and ``all`` options as the CLI command:
...
can check whether it found anything to kill:
