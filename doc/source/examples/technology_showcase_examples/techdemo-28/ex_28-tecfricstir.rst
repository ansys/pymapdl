...
This example problem shows how to simulate the FSW process.
Several characteristics of FSW are presented, including
...
You can also perform this example analysis entirely in Ansys Mechanical.
For more information, see "Friction Stir Welding (FSW) Simulation"
in *Workbench Technology Showcase: Example Problems*.

Introduction
------------

FSW is a solid-state welding technique that involves
...
thermal `Boundary conditions and loading`_ are applied on the model. The
simulation occurs over three load steps, representing the `Loading`_ of
the process.
...
the welded plates via a 3D elastoplastic thermomechanical simulation.
...
**Figure 28.1: 3D Model of Workpiece and Tool**
...
Modeling is a two-part task:

*  `Workpiece and tool modeling`_
*  `Contact modeling`_

Workpiece and tool modeling
^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
region. This figure shows the 3D meshed model:
...
**Figure 28.2: 3D Meshed Model of Workpiece and Tool**


Contact modeling
^^^^^^^^^^^^^^^^

For an FSW simulation a contact is modeled as follows:

* Contact pair between the plates
* Contact pair between the tool and workpiece
...
During the simulation, the surfaces to be joined come into contact. This figure shows
a standard surface-to-surface contact pair using ``TARGE170`` and ``CONTA174``:
...
defined between the tool and workpiece. This figure shows that the ``CONTA174``
element is used to model the contact surface on the top surface of the workpiece and the
``TARGE170`` element is used for the tool:
...
surface of the tool to apply the rotation and translation on the tool.
...
It is assumed that the plastic deformation of the material uses the von Mises
...
The following code sets up the material properties:
...
*  `Thermal boundary conditions`_
*  `Mechanical boundary conditions`_
*  `Loading`_
...
workpiece, the convection and radiation account for heat loss to the
...
*  `Deformation and stresses`_
*  `Temperature results`_ 
*  `Welding results`_
*  `Heat generation`_
...
line during the FSW process. This figure shows the deflection of the
...
Following load step 1, the temperature remains unchanged (25 °C):
...
and increase rapidly. The next two figures show the increment in contact
...
The following two figures show the temperature rise due to heat generation in the
...
This figure shows the temperature distributions on the top surface of
...
This figure shows the temperature distribution in the thickness direction
at location 1:
...
time-step (:meth:`Mapdl.etable <ansys.mapdl.core.Mapdl.etable>`). This
figure shows the plot of total frictional heat generation rate on the
workpiece with time:
