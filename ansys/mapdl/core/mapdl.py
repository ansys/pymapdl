...
class _MapdlCore(Commands):
    ...
    def load_array(self, name, array):
        ...
        logger.info(f"Generating file for table in {filename}")
    ...
    def load_table(self, name, array, var1="", var2="", var3="", csysid=""):
        ...
        if any(array[0, :] != np.sort(array[0, :])):
            raise ValueError('The underlying ``TREAD`` command requires that the axis 0 is in ascendent order.')
        if any(array[:, 0] != np.sort(array[:, 0])):
            raise ValueError('The underlying ``TREAD`` command requires that the axis 1 is in ascendent order.')
