...
def check_license_server_with_ansysli_util():
    ...
    """
    warnings.warn(
        "This method to check the license server status is not completely error free.\nIt is very likely it will show license not available."
    )
    ...
    for each_license in licenses:
        ...
        if msg1 in output or msg2 in output:
            raise LicenseServerConnectionError(
                head_message=f"'ansysli_util' reports '{msg1}' or '{msg2}'"
            )
...
def try_license_server():
    ...
    if not status:
        status = try_ping_server_python()

    if not status:
        status = try_ansysli_util()
