...
def launch_mapdl(
    ...
    **kwargs,
    ...
    if "smp" not in additional_switches:  # is distributed
        if _is_ubuntu():
            os.environ['I_MPI_SHM_LMT'] = 'shm'
    
        if 'force_intel' not in additional_switches:
            ...
            if 'intelmpi' in additional_switches:
                # Remove intel flag.
                regex = "(-mpi)( *?)(intelmpi)"
                additional_switches = re.sub(regex, '', additional_switches)
        ...
        elif 'force_intel' in additional_switches:
            # Removing force_flag since it is not an ANSYS parameter.
