"""
.. _ref_ex_0-tecbrakesqueal_1:

Example Problem 1: Brake Squeal Analysis
----------------------------------------
...
.. figure:: ../../../../_images/gtecbrakesqueal_fig1.gif
    :align: center
...
def start(mapdl, verify):
    mapdl.finish()
    mapdl.run("/batch,list")
    mapdl.run(f"/verify,{verify}")
    mapdl.run("/show,png")
...
#
# .. figure:: ../../../../_images/gtecbrakesqueal_fig5.gif
#     :align: center
...
mapdl.set(1, 21)
# mapdl.plnsol("u", "sum")  # Replaced by the most next function
...
mapdl.post_processing.plot_nodal_displacement(
    ...
    cpos="xy",
    scalar_bar_args={"title": "Displacement", "vertical": False},
) # Plot the mode shape for mode 22
...
# **Static Solution**
#
mapdl = start(mapdl, verify="partial_prestressed")
...
def plot_element_results(mapdl, item, comp, title, show_elem_numbering=False):
    scalars = mapdl.post_processing.element_values(item, comp=comp)
    kwargs = {"scalar_bar_args": {'title': title}}
    mapdl.post_processing._plot_cell_scalars(
...
plot_element_results(mapdl, "CONT", "STAT", "Contact status")

# mapdl.plesol("cont", "slide")  # Plot the slide values for the selected contact elements
plot_element_results(mapdl, "CONT", "SLIDE", "Contact sliding distance")
...
plot_element_results(mapdl, "U", "NORM", "Total displacement substep 21")

mapdl.set(1, 22)
# mapdl.plnsol("u", "sum")  # Plot the mode shape for mode 22
plot_element_results(mapdl, "U", "NORM", "Total displacement substep 22")
...
#
mapdl = start(mapdl, verify='full_non_linear')
...
#
mapdl.run("/SOLU")
mapdl.antype("STATIC", "RESTART", "", "", "PERTURB")  # Restart from last load step and sub step
