...
Description:
The 11 blade model along with a fictious disk is modeled. CFX generated
...
mapdl.title("Verify Pressure Data Mapping exported from CFX on Structural 11 Blade Model")
...
def my_mapdl_launch_in_cwd(my_wdirnew, my_job_name):
    ...
    try:
        ...
        pass
    exec_file_222 = os.path.join(os.getenv('ANSYS222_DIR'), 'bin', 'winx64', 'ansys222.exe')
    mapdl = launch_mapdl(run_location=new_wdir_path, jobname=my_job_name,
                         nproc=4, override=True, exec_file=exec_file_222)
...
def aero_blade_cfx_pres_mapping(mapdl):
    ...
    mapdl.run("target,pressure_faces")                  # Specifies the target nodes for mapping pressures onto surface effect elements.
    mapdl.ftype(filetype='cfxtbr', prestype='1')        # Specifies the file type and pressure type for the subsequent import of source points and pressures.
    mapdl.read(fname='11_blades_mode_1_ND_0.csv')       # Read CFX exported file (*.csv file) Blade 2, Export Surface 1
    # ngen,2,0,all,,, 0,360*3/22   	                    # Compatible with blade 1
    print(mapdl.map(kdim='2', kout='1'))                # Maps pressures from source points to target surface elements.
