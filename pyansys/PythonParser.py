...
def StoreTet(offset, ecount, ccount, cells, cell_type, numref, elem, i, lin):
    ...
    # edge nodes
    # [0, 1, 2, 2, 3, 3, 3, 3]
    cells[ccount] = numref[elem[i, 0]]
    ccount += 1
    cells[ccount] = numref[elem[i, 1]]
    ccount += 1
    cells[ccount] = numref[elem[i, 2]]
    ccount += 1
    cells[ccount] = numref[elem[i, 4]]
    ccount += 1

    # midside nodes
    if not lin:
        cells[ccount] = numref[elem[i, 8]]
        ccount += 1
        cells[ccount] = numref[elem[i, 9]]
        ccount += 1
        cells[ccount] = numref[elem[i, 11]]
        ccount += 1
        cells[ccount] = numref[elem[i, 16]]
        ccount += 1
        cells[ccount] = numref[elem[i, 17]]
        ccount += 1
        cells[ccount] = numref[elem[i, 18]]
        ccount += 1
...
def StoreWeg(offset, ecount, ccount, cells, cell_type, numref, elem, i, lin):
    ...
    cells[ccount] = numref[elem[i, 0]]
    ccount += 1
    cells[ccount] = numref[elem[i, 6]]
    ccount += 1
    cells[ccount] = numref[elem[i, 5]]
    ccount += 1
    cells[ccount] = numref[elem[i, 4]]
    ccount += 1
    ...
    if not lin:
        ...
        cells[ccount] = numref[elem[i, 11]]
        ccount += 1
        cells[ccount] = numref[elem[i, 13]]
        ccount += 1
        cells[ccount] = numref[elem[i, 12]]
        ccount += 1
        cells[ccount] = numref[elem[i, 15]]
        ccount += 1
        cells[ccount] = numref[elem[i, 18]]
        ccount += 1
        cells[ccount] = numref[elem[i, 17]]
...
def Parse(raw, force_linear):
    ...
    cells = np.empty(nelem * (20 + 1), np.int64)
    ...
    return np.asarray(cells[:ccount]), np.asarray(
        offset[:ecount]), np.asarray(cell_type[:ecount]), np.asarray(numref)
...
def ParseForFEM(raw):
    ...
    # sized for maximum number of nodes, will trim later
    midedgeind = np.empty((nnode, 2), np.int64)

    # Also track the original midside node indices
    # sized for maximum number of nodes, will trim later
    midind = np.empty(nnode, np.int64)
    midnode = 0
    ...
    for i in range(nelem):
                ...
                if elem[i, 6] != elem[i, 7]:  # check if hexahedral
                    ...
                    for k in range(12):
                        ...
                        if elem[i, k + 8] > 0 and not midstored[elem[i, k + 8]]:
                            ...
                            midedgeind[midnode,
                                       0] = numref[elem[i, typeAmidind[k][0]]]
                            midedgeind[midnode,
                                       1] = numref[elem[i, typeAmidind[k][1]]]
                    ...
                    for k in range(9):
                        # if it exists
                        if elem[i,
                                wegmid[k] + 8] > 0 and not midstored[elem[i,
                                                                          wegmid[k] + 8]]:
                            # Add its adjcent nodes to the index array tracker
                            midedgeind[midnode, 0] = numref[elem[i,
                                                                 typeAmidind[wegmid[k]][0]]]
                            midedgeind[midnode, 1] = numref[elem[i,
                                                                 typeAmidind[wegmid[k]][1]]]
                    ...
                    for k in range(8):
                        # if it exists
                        if elem[i,
                                pyrmid[k] + 8] > 0 and not midstored[elem[i,
                                                                          pyrmid[k] + 8]]:
                            # Add its adjcent nodes to the index array tracker
                            midedgeind[midnode, 0] = numref[elem[i,
                                                                 typeAmidind[pyrmid[k]][0]]]
                            midedgeind[midnode, 1] = numref[elem[i,
                                                                 typeAmidind[pyrmid[k]][1]]]
                    ...
                    for k in range(6):
                        # if it exists
                        if elem[i,
                                tetmid[k] + 8] > 0 and not midstored[elem[i,
                                                                          tetmid[k] + 8]]:
                            # Add its adjcent nodes to the index array tracker
                            midedgeind[midnode, 0] = numref[elem[i,
                                                                 typeAmidind[tetmid[k]][0]]]
                            midedgeind[midnode, 1] = numref[elem[i,
                                                                 typeAmidind[tetmid[k]][1]]]
        ...
        for j in range(2):
            if elem_type[etype[i]] == typeB[j]:
                ...
                # Poulate midside nodes
                for k in range(6):
                    # if it exists (isn't zero because ANSYS is weird) and
                    # hasn't been
                    if elem[i, tetmidB[k]] > 0 and midstored[elem[i, tetmidB[k]]]:
                        # Add its adjcent nodes to the index array tracker
                        midedgeind[midnode,
                                   0] = numref[elem[i, typeBmidind[k][0]]]
                        midedgeind[midnode,
                                   1] = numref[elem[i, typeBmidind[k][1]]]
