...
class Mapdl(_MapdlCommands, _DeprecCommands):
    ...
    - aas :  Enables server mode. When enabling server mode, a custom
             ...
             option. For more information, see Mechanical APDL as a
             Server User's Guide.
    ...
    -amfg : Enables the additive manufacturing capability (requires an
     additive manufacturing license). For general information about
    ...
    -b list or nolist : Activates the Mechanical APDL program in batch
     mode. The options -b list or -b by itself cause the input listing
     to be included in the output. The -b nolist option causes the
     input listing not to be included. For more information about
     running Mechanical APDL in batch mode, see Batch Mode.
    ...
    -dir : Defines the initial working directory. Using the -dir
     option overrides the ANSYS201_WORKING_DIRECTORY environment
     variable.
    ...
    -g : Launches the Mechanical APDL program with the Graphical User
     ...
     different device. This option is not used on Windows systems. To
     activate the GUI after Mechanical APDL has started, enter two
     commands in the input window: /SHOW to define the graphics
     device, and /MENU,ON to activate the GUI. The -g option is valid
     only for interactive mode.  Note: If you start Mechanical APDL
     via the -g option, the program ignores any /SHOW command in
     the start.ans file and displays a splash screen briefly before
     opening the GUI windows.
    
    -i inputname : Specifies the name of the file to read input into
     Mechanical APDL for batch processing. On Linux, the preferred method
     to indicate an input file is <.
    ...
    -j Jobname : Specifies the initial jobname, a name assigned to all
     files generated by the program for a specific model. If you omit
     the -j option, the jobname is assumed to be file.
    ...
    -l language : Specifies a language file to use other than US
     ...
     /ansys_inc/v201/ansys/docu (or Program Files\ANSYS
     Inc\V201\ANSYS\docu on Windows systems).

    -m workspace : Specifies the total size of the workspace (memory)
     in megabytes used for the initial allocation. If you omit the -m
     option, the default is 2 GB (2048 MB). Specify a negative number
     (-value) to force a fixed size throughout the run.
    ...
    -o outputname : Specifies the name of the file to store the output
     from a batch execution of Mechanical APDL. On Linux, the
    ...
    -s read or noread : Specifies whether the program reads the
     start.ans file at start-up. If you omit the -s option, Mechanical
     APDL reads the start.ans file in interactive mode and not in
     batch mode.
    ...
    -scname <name of the solver> : Specifies the unique name used by the
     co-simulation participant to identify itself to the coupling
     service in a System Coupling analysis. For Linux systems, you
     need to quote the name to have the name recognized if it contains
     a space: (e.g. ``ansys201 -scname "Solution 1"``)
    ...
    -usersh : Directs the MPI software (used by Distributed ANSYS) to
     use the remote shell (rsh) protocol instead of the default secure
     shell (ssh) protocol. See Configuring Distributed ANSYS in the
     Parallel Processing Guide for more information.
