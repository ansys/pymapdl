    ...
    with open(filename) as fileObj:
        ...
        for line in fileObj:
            ...
            if 'ET,' in line[:3]:
                ...
                nfpos, fpos, n_isz, nfields, n_fsz, nnode = NBLOCK_Info(
                    fileObj, line, fpos)
...
#    if nfpos < efpos and nfpos != -1:
    if nfpos != -1:
        nnum, nodes = NBLOCK_Reader(
            filename, nfpos, n_isz, nfields, n_fsz, nnode)
...
def GetBlockFormat(formatstring):
    ...
    # Digit Size
    d_size = int(formatstring[formatstring.find(
        'i') + 1:formatstring.find(',')])
    f_size = int(formatstring[formatstring.find(
        'e') + 1:formatstring.find('.')])
